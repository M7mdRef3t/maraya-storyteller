...
export default function NarrationText({
  ...
  onComplete,
  onBlockStart,
  speed = 45, // ms per word (roughly 22 words per sec)
}) {
  ...
  useEffect(() => {
    ...
    const playBlock = (blockIndex) => {
      ...
      // Calculate dynamic interval based on text length to keep pacing cinematic
      const intervalMs = Math.max(30, speed + (totalWords > 30 ? -10 : 0));
      ...
      timerRef.current = setInterval(() => {
        setActiveWordCount((prev) => {
          ...
          if (next >= totalWords) {
            ...
            // Wait slightly before showing the next block or finishing
            setTimeout(() => {
            ...
            }, 600); // 600ms dramatic pause between blocks
          }
          return next;
        });
      }, intervalMs);
